 ...
 */
/**
 * Build the full Docker run spec for an agent-job container WITHOUT actually
 * creating the volume or starting anything. Returns everything a Docker host
 * (local OR a remote conclave node-worker) needs to launch the container.
 *
 * Used by both runAgentJobContainer (local path) and the conclave dispatch
 * path in lib/tools/create-agent-job.js. Keep these two callers in sync by
 * always going through this builder — never construct the env array twice.
 *
 * Side effects: issues a per-container API key in the central DB. Both the
 * local and remote paths need this so the agent can call back to APP_URL.
 *
 * @returns {Promise<{
 *   image: string,
 ...
 * }>}
 */
async function buildAgentJobContainerSpec({ agentJobId, repo, branch, title, description, codingAgent, llmModel }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  const image = `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
  ...
  // APP_URL so agents can call back to the event handler
  const appUrl = getConfig('APP_URL');
  if (appUrl) env.push(`APP_URL=${appUrl}`);

  // Per-container API key for agent-secrets access
  const { createAgentJobApiKey } = await import('../db/api-keys.js');
  const { key: agentJobToken } = createAgentJobApiKey(containerName);
  env.push(`AGENT_JOB_TOKEN=${agentJobToken}`);

  // Plain agent_job_secret values as env vars (oauth types are null — agent must fetch via /get)
  const { getAllAgentJobSecrets } = await import('../db/config.js');
  for (const { key, value } of getAllAgentJobSecrets()) {
    if (value !== null && !env.some((e) => e.startsWith(`${key}=`))) {
      env.push(`${key}=${value}`);
    }
  }
  ...
  return { image, containerName, volumeName, env, hostConfig, agent, backendApi, shortId };
}

async function runAgentJobContainer(opts) {
  const spec = await buildAgentJobContainerSpec(opts);

  // Create the named workspace volume on this Docker host before starting
  // the container. (For remote dispatch the worker creates its own volume.)
  await createVolume(spec.volumeName);

  console.log(`[agent-job] id=${spec.shortId} agent=${spec.agent} image=${spec.image} backendApi=${spec.backendApi} (local)`);
  ...
  return { ...result, volumeName: spec.volumeName, backendApi: spec.backendApi };
}
